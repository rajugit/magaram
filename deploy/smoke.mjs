...
await check('/preview/8', 200);
const ads = (await (await check('/api/v1/ads/public', 200)).json()).data;
assert.deepEqual(ads, { items: [], deliveryEnabled: false });
await check('/api/v1/ads/campaigns', 401);
...
const adsStatus = (await (await check('/api/v1/ads/status', 200, { headers })).json()).data;
assert.equal(adsStatus.paymentEnabled, false);
assert.equal(adsStatus.deliveryEnabled, false);
await check('/api/v1/ads/campaigns', 200, { headers });
await check('/api/v1/ads/placements', 200, { headers });
